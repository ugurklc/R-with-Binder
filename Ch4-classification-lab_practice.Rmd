...
```{r chunk4}
glm.fit = glm( Predicted variable ~ Sum of Predictors, data= , family=    )
summary(glm.fit)
```
...
```{r chunk6}
glm.probs = predict(output of prediction model, type = )
```
...
```{r chunk9}
train = (Year <    )
Smarket.2005 = keep only 2005 in Smarket
dim(Smarket.2005)
```
...
The `!` symbol can be used to reverse all of the elements of  a Boolean vector. That is, `!train` is a vector similar to `train`, except that  the elements that are `TRUE` in `train` get swapped to `FALSE` in `!train`, and the elements that are `FALSE` in `train` get swapped to `TRUE` in `!train`. Therefore, `Smarket[!train, ]` yields a submatrix of the stock market data containing only
...
```{r chunk10}
glm.fits = glm( Predicted ~ sum of predictors, data = , family =    , subset =  training set  )
```


```{r}
glm.probs = predict(model, test set, type = "...")
```
...
```{r chunk13}
predict(model, newdata = dataframe here, type = "  ")
```
...
```{r chunk14}
library(MASS)
lda.fit <- lda( Predicted ~ sum of predictors, data =   , subset = training set)
lda.fit
plot(lda.fit)
```
...
The LDA output indicates that $\hat\pi_1=0.492$ and $\hat\pi_2=0.508$; in other words, $49.2\%$ of the training observations correspond to days during which the market went down.
 ...
 These suggest that there is a tendency for the previous 2~days' returns to be negative on days when the market increases, and a tendency for the previous days' returns to be positive on days when the market declines.
  The *coefficients of linear discriminants* output provides the linear combination of `Lag1` and `Lag2` that are used to form the LDA decision rule. In other words, these are the multipliers of the elements of $X=x$ in ( 4.24).
...
```{r chunk15}
lda.pred <- predict(model, test set)
names(lda.pred)
```
...
```{r chunk22}
library(e1071)
nb.fit <- naiveBayes(Predicted ~ sum predictors, data = ,
    subset =  )
nb.fit
```

The output  contains the estimated mean and standard deviation for each variable in each class. For example, the mean for `Lag1` is $0.0428$ for 
...
```{r chunk24}
nb.class <- predict(model , test set)
table(nb.class, test set)
mean(nb.class == test values)
```
...
```{r chunk25}
nb.preds <- predict(model, test set, type = "raw")
nb.preds[1:5, ]
```
...
```{r chunk27}
set.seed(1)
knn.pred <- knn(train set , test set , train variable , k = 1)
table(knn.pred, test variable)
(83 + 43) / 252
```
